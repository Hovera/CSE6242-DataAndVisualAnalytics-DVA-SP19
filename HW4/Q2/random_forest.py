...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        feature_len = len(XX[0][0:-1])
        # generate sample indices
        random_indices = np.random.choice(range(n), size=n, replace=True)
        # create new matrices based on indices generated
        for index in random_indices:
            samples.append(XX[index][:feature_len])
            labels.append(XX[index][feature_len])
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                new_vote = []
                # for number of trees, append records into vote
                for i in self.decision_trees:
                    new_vote.append(i.classify(record))
                # append max vote into y
                y = np.append(y, np.argmax(np.bincount(new_vote)))
            else:
